    ...
    <div class="sources-body">
      ...
      <section class="sources-section">
        ...
        <ul class="sources-list">
          ...
          <li class="sources-item">
            <div class="sources-item-name">Aggregate graph — station anomaly baseline</div>
            <div class="sources-item-desc">In anomaly modes (Monthly Anomaly, Annual Anomaly) each station's temperatures are expressed as departures from a per-station, per-calendar-month baseline before aggregation. The baseline is derived from up to 30 "full years" — years where all 12 months are non-missing — selected to be as close as possible to the centre of the station's record. The record midpoint is defined as the average of the first and last years with any data. Full years are ranked by absolute distance from this midpoint, with ties resolved in favour of earlier years, and the nearest 30 are used. Stations that have no full years at all are excluded from the anomaly aggregation. The monthly baseline values are subtracted from every observed month, so the resulting time series reflects departures from each station's own climatological normal rather than raw temperatures. This removes the effect of stations entering or leaving the network and makes multi-station means more comparable across time. <span class="sources-cite">No external DOI; project-specific method note.</span></div>
          </li>
          <li class="sources-item">
